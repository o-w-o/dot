...
@Slf4j
class AliyunOssServiceTest extends ResourceServiceTest {
  @Autowired
  ...
  public void testOssObjectWithStyle() {
    var result = this.aliyunStsService.createStsCredentialsForUser(
        ...
        AuthorizedUser.anonymousUser(this.myIp).setUsername("Test").setId(5)
    ).guard();
    ...
    GeneratePresignedUrlRequest req = new GeneratePresignedUrlRequest(this.myOssProperties.getBucketName(), this.myOssProperties.getPublicDir() + "/logo-x.png", HttpMethod.GET);
