...
@Slf4j
@Component
@FieldTypeSelector(FieldType.TypeEnum.TEXT)
public class TextFieldSpaceHandler extends FieldSpaceHandler {

  @Resource
  FieldSpaceRepository<TextSpace> textSpaceFieldSpaceRepository;

  @Resource
  FieldSpaceHandlerFactory<TextSpace> fieldSpaceHandlerFactory;
  FieldSpaceDefaultHandler<TextSpace> fieldSpaceDefaultHandler;

  @Resource
  private TextFieldSpacePayloadHandlerHolder textFieldSpacePayloadHandlerHolder;
  ...
  public ServiceResult<Field> process(Field field) {
    var space = (TextSpace) field.getSpace();
    logger.info("field -> [{}], payload -> [{}]", space, space.getPayload());

    textFieldSpacePayloadHandlerHolder.select(space.getPayloadType().getType()).ifPresent(handler -> {
